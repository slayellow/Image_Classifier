...
class Block(nn.Module):
    def __init__(self, in_channel, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.pwconv1 = nn.Conv2d(in_channel, 4*in_channel, kernel_size=1, bias=False)
        self.act = nn.GELU()
        self.pwconv2 = nn.Conv2d(4 * in_channel, in_channel, kernel_size=1, bias=False)
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, version, num_classes=1000):
        ...
        stem = nn.Sequential(nn.Conv2d(in_channel, channels[version][0], kernel_size=4, stride=4, bias=False),
                             LayerNorm(channels[version][0], eps=1e-6))
        self.downsample_layers.append(stem)

        # Downsampling Stage Between stem, res2, res3, res4, res5
        for i in range(3):
            downsample_layer = nn.Sequential(
                LayerNorm(channels[version][i], eps=1e-6),
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(channels[version][j], drop_path=dp_rates,
